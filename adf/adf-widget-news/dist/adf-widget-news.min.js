    ...
    angular.module("adf.widget.news", ["adf.provider"])
        ...
        .config(["dashboardProvider", function (e)
            ...
            e.widget("news", {
                    ...
                    resolve: {feed: ["newsService", "config", function (e, t) {return t.url ? e.get(t.url) : void 0}]},
        ...
        .service("newsService", ["$q", "$http", "newsServiceUrl", function (e, t, r) {
            return {
                get: function (n) {
                    var l = e.defer();
                    return t.jsonp(r + encodeURIComponent(n)).success(function (e) { e && e.responseData && e.responseData.feed ? l.resolve(e.responseData.feed) : l.reject() })
                        .error(function () { l.reject() }), l.promise} }
            }])
        .controller("newsCtrl", ["$scope", "feed", function (e, t) { e.feed = t }]),
            ...
            e.put("{widgetsPath}/news/src/edit.html", '<form role=form><div class=form-group> ' +
                ...
                '</div></div></form>'),
            //e.put("{widgetsPath}/news/src/view.html", '<div class=news><div class="alert alert-info" ng-if=!feed>Please insert a feed url in the widget configuration</div><h4><a ng-href={{feed.link}} target=_blank>{{feed.title}}</a></h4><ul><li ng-repeat="entry in feed.entries"><a ng-href={{entry.link}} target=_blank>{{entry.title}}</a></li></ul></div>')
