        ...
        .controller("newsCtrl", ["$scope", "feed", function (e, t) {
            ...
            angular.module("adf.widget.news").run(["$templateCache", function (e) {
                ...
                e.put("{widgetsPath}/news/src/edit.html", '<form role=form><div class=form-group> ' +
                    ...
                    '</div></div></form>'),
                e.put("{widgetsPath}/news/src/view.html", '<div class=news><div class="alert alert-info" ng-if=!feed>Please insert a feed url in the widget configuration</div><ul><li ng-repeat="entry in feed.entries"><a ng-href={{entry.link}} target=_blank>{{entry.title}}</a></li></ul></div>')
